...
const grid = layout = input.split("\n").map(line => line.split(""))

const MAX_X = layout[0].length - 1
const MAX_Y = layout.length - 1

const validatePosition = ([y, x]) => {
    return x >= 0 && x <= MAX_X && y >= 0 && y <= MAX_Y
}

const directAdjacent = (grid, pos) => {
    let current = grid[pos[0]][pos[1]]

    const occupied = directions
        // Possible seats
        .map(([y, x]) => [pos[0] + y, pos[1] + x])
        .filter(validatePosition)
...
const indirectAdjacent = (grid, pos) => {
    const occupied = directions
        // Possible seats
        .reduce((acc, [y, x]) => {
            py = pos[0] + y
            px = pos[1] + x
            while (validatePosition([py, px])) {
                if (grid[py][px] !== '.') {
                    acc.push(grid[py][px])
                    break
                }
    ...
    const current = grid[pos[0]][pos[1]]

    if (current == 'L' && occupied == 0) {
        return '#'
    } else if (current == '#' && occupied >= 5) {
        return 'L'
    }
    return current
}
...
const run = (grid, callback) => {
    ...
    do {
        ...
        occupied = 0
        currentGrid = currentGrid.map((line, y) => {
            return line.map((value, x) => {
                let newValue = callback(currentGrid, [y, x])
                if (value !== newValue) {
                    done = false
                }
                if (newValue === '#') {
...
console.debug('Part 1:', run(grid, directAdjacent))
console.debug('Part 2:', run(grid, indirectAdjacent))
